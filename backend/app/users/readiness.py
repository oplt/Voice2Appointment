...
def compute_readiness(db: Session, user: User) -> dict[str, Any]:
    ...
    add(
        ...
        policy.default_service_duration_minutes >= 5,
        fix_path="/settings",
        detail=f"Default duration {policy.default_service_duration_minutes} minutes.",
    )
    add(
        "business_hours",
        ...
        detail=(
            ...
            else "Optional — empty hours mean unrestricted booking."
        ),
        required=False,
    )
    add(
        "telephony",
        "Twilio credentials",
        bool(user.twilio_account_sid and user.twilio_auth_token),
        fix_path="/settings",
        detail=(
    ...
    add(
        ...
        bool(user.twilio_phone_number),
        fix_path="/settings",
        detail=user.twilio_phone_number or "Set the Twilio phone number that routes here.",
    )
    add(
        "deepgram",
