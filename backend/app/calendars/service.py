...
def calendar_status(db: Session, user_id: int) -> dict[str, Any]:
    ...
    cache_key = durable_versioned_key(db, user_id, "cal", "status")
    ...
    if not auth:
        ...
        payload = {
            ...
            # Embed links are generated by the dedicated endpoint so they are
            # fresh and validated instead of serving a stale stored URL.
            "embedded_link": None,
            "status": auth.status,
        }
...
def list_events(
    ...
    auth = get_auth_record(db, user_id)
    zone_name = (
        timezone_str or (auth.time_zone if auth else None) or settings.default_timezone or "UTC"
    ).strip()
    try:
        user_tz = ZoneInfo(zone_name)
    ...
    if end_date - start_date > timedelta(days=settings.calendar_max_range_days):
        raise ValueError(
            f"calendar range cannot exceed {settings.calendar_max_range_days} days"
        )
    canonical_start = start_date.astimezone(timezone.utc).isoformat()
    canonical_end = end_date.astimezone(timezone.utc).isoformat()
    # Keep the requested IANA zone in the key even when its current offset is
    # UTC (for example Europe/London in winter).  Offset-based aliases would
    # otherwise split or merge semantically distinct calendar views.
    canonical_zone = user_tz.key
    ...
    if isinstance(cached, list):
        return {"items": cached, "effective_timezone": canonical_zone}
    ...
    for event in events:
        if not isinstance(event, dict) or not isinstance(event.get("start"), dict) or not isinstance(event.get("end"), dict):
            raise ValueError("calendar provider returned malformed event")
        start = event["start"].get("dateTime") or event["start"].get("date")
        end = event["end"].get("dateTime") or event["end"].get("date")
        if not isinstance(event.get("id"), str) or not isinstance(start, str):
            raise ValueError("calendar provider returned malformed event")
    ...
    return {"items": formatted, "effective_timezone": canonical_zone}
