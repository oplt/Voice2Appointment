...
def mask_phone_label(value: str | None) -> str:
    """Mask phone numbers for default analytics (last 4 digits only)."""
    if not value:
        return "***"
...
def aggregate_twilio_sql(
    ...
    *,
    ...
    # ColumnElement shapes differ by dialect; keep as Any for grouping expressions.
    day_expr: Any
    hour_expr: Any
    dow_expr: Any
    if dialect == "postgresql":
        ...
        day_expr = cast(TwilioCall.start_time, Date)
    ...
    cost_labels = [str(r.day) for r in cost_rows]
    cost_values = [round(_as_float(r.cost), 4) for r in cost_rows]
    ...
    top_numbers = {
        "labels": [mask_phone_label(r.to_number) for r in top_rows],
    ...
    from_rows = db.execute(
        select(TwilioCall.from_number, func.count().label("n"))
        .where(*base, TwilioCall.from_number.is_not(None))
        .group_by(TwilioCall.from_number)
    ).all()
    top_countries, geo_counts = _countries_from_numbers(
        [(r.from_number, int(r.n)) for r in from_rows]
    )
    ...
    return {
        ...
        "cost_over_time": {"labels": cost_labels, "values": cost_values},
...
def _countries_from_numbers(
    ...
    for raw, n in pairs:
        ...
        if not country:
            continue
        buckets[country.alpha_3] += n

    ranked = []
    for iso3, calls in buckets.items():
